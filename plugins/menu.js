...
/*********************** SOURCE ************************/
  ...
  let logo2 = fs.readFileSync('./src/RadBotZ.jpg')
  let logo3 = fs.readFileSync('./src/RadBotZ.jpg')
  kannaImg = (await conn.prepareMessage('0@s.whatsapp.net', fs.readFileSync('./src/RadBotZ.jpg'), MessageType.image, { thumbnail: Buffer.alloc(0) })).message.imageMessage
...
let ftt = `©️ Recode By Alpin`
    ...
    // BUTTON VIDEO
    await conn.send2ButtonImg(m.chat, await (await fetch(`${logos()}`)).buffer(), '───────[ *DASHBOARD* ]────────', text, 'OWNER', '.owner', 'DONASI', '.donasi', fkon, { contextInfo: { mentionedJid: conn.parseMention(text), forwardingScore: 999, isForwarded: true}})
